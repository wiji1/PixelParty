...
public class PixelScoreboard {
	...
	public PixelScoreboard() {
		this.scoreboard = Bukkit.getScoreboardManager().getNewScoreboard();
		this.title = ChatColor.translateAlternateColorCodes('&', "&e&lPIXEL PARTY");
	...
	private void applyText(Team team, String text, OfflinePlayer result) {
		...
		if(text.length() > 16) {
			...
			if(suffix.length() > 16) {
				suffix = suffix.substring(0, (13 - prefixColor.length())); // cut off suffix, done iftext is over 30 characters
			}
